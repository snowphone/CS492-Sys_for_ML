...
\lstinputlisting[multicols=2, frame=none, basicstyle=\ttfamily\tiny]{../__init__.py}
...
\begin{table}[h!]
	...
	\label{tab:eval}
\end{table}
The total time is that whole time of 23.89096s on GPU and 44.00026s on CPU. End-to-end time includes preprocessing input frame, inference, post processing, and writing the result clip.
...
Interesting result was shown at the beginning of the ‘session.run’. On CPU, it took 0.15 seconds to inference the first frame, while the average time was 0.05979s.
Moreover, on the GPU, it took 1.252 seconds for the inference first frame and 0.103 for the second, while the average was 0.01539. 
