		...
		while (len(currents) != 0):
			...
			for current in currents:
				...
				if skip_current:
					continue
				current.run(counter)
				print("layer: {}".format(current.name))
...
class Conv2D(DnnNode):
	def __init__(self, name, in_node, kernel, strides, padding):
		...
		# weights
		self.weights = kernel.astype(np.float32, order = "C")
		...
		self.result = np.zeros((1, self.OW, self.OH, self.OC)).astype(np.float32)

	def run(self, counter):
		ptin = np.pad(self.in_node.result, self.pad, mode='constant').astype(np.float32)
